...
global alph0_global, pop_global, hemisphere_global, flg_ALC_global
...
# -------------------------------------------------------------------------------------------
def precipitation_spectrum(vlsvReader=None,cid=None,losscone=None,pop=None,emin=None,emax=None,hemisphere=None,flg_antilosscone=False):
...
def plot_prec_spectrum(filename=None,
    ...
    :kword flg_antilosscone: Set to True to plot "anti-precipitation" instead (i.e., flux associated with particles in the anti loss cone)
    ...
    if cellidplot==None:
        ...
        for cellid in cellidplot:
            ...
            # Output file name
            if flg_antilosscone:
                savefigname = outputdir+run+"_prec_spec"+stepstr+"_"+str(cellid)+"antiLossCone.png"
            else:
            ...
            (wentFine,energy,flux) = precipitation_spectrum(vlsvReader=f,cid=cellid,losscone=alph0,pop=pop,emin=emin,emax=emax,hemisphere=hemisphere,flg_antilosscone=flg_antilosscone)
        ...
        if flg_antilosscone:
            datamap.dump(outputdir+'datamap_cid'+str(cellidplot[0])+'_antiLossCone')
            np.save(outputdir+'latitudes_keogram_'+str(cellidplot[0])+'_antiLossCone',latitudes)
            np.save(outputdir+'energy_scale_'+str(cellidplot[0])+'_antiLossCone',energy)
        else:
...
def make_keogram_column(step=None):
    global alph0_global, linedipole_global, fluxdir_global, flg_ALC_global
    ...
    (wentFine,energy,flux) = precipitation_spectrum(vlsvReader=f,cid=cellid_global,losscone=alph0,pop=pop_global,
                                                    emin=emin_global,emax=emax_global,hemisphere=hemisphere_global,flg_antilosscone=flg_ALC_global)
...
def plot_prec_time_spectrum(filedir=None,
    ...
    emin_global=emin
    emax_global=emax
    flg_ALC_global=flg_antilosscone
    ...
    # Output file name
    if flg_antilosscone:
        savefigname = outputdir+run+"_prec_spec_time"+stepstr+"_"+str(cellidplot[0])+"_antiLossCone.png"
    else:
    ...
    # Save the keogram to file in case further processing is needed
    if flg_antilosscone:
        datamap.dump(outputdir+'datamap_cid'+str(cellidplot[0])+'_'+stepstr+'_antiLossCone')
        np.save(outputdir+'time_keogram_cid'+str(cellidplot[0])+'_'+stepstr+'_antiLossCone',time_keogram)
        np.save(outputdir+'energy_scale_'+str(cellidplot[0])+'_antiLossCone',energy)
    else:
